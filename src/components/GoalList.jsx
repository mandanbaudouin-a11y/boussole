...
function StrategyChip({ strategy, canEdit, onRemove }) {
  return (
    <span className="strategy-chip">
      {strategy.category && (
        <span className="strategy-category">{STRATEGY_CATEGORY_LABELS[strategy.category]}</span>
      )}
      {strategy.label}
      {canEdit && (
        <button className="strategy-chip-remove" onClick={onRemove} title="Retirer">
          &times;
        </button>
      )}
    </span>
      ...
      >
        <div style={{ position: 'relative', flex: 1, minWidth: 160 }}>
          <input
            ...
            style={{ width: '100%' }}
            placeholder="Ajouter une stratégie (suggestion ou texte libre)"
            value={value}
  ...
  return (
    <div style={{ marginTop: -2, marginBottom: 8 }}>
      <button className="status-history-toggle" onClick={() => setOpen((v) => !v)}>
        ...
        {strategyCount > 0 && ` · ${strategyCount} stratégie${strategyCount > 1 ? 's' : ''}`}
        {historyCount > 0 && ` · historique (${historyCount})`}
      </button>
      {open && (
        <div className="status-history-list">
          ...
          {strategyCount === 0 && (
            <p style={{ margin: '0 0 8px', color: 'var(--ink-soft)' }}>Aucune stratégie associée.</p>
          )}
          {strategyCount > 0 && (
          ...
          {historyCount > 0 && (
            <>
              <p className="report-section-title" style={{ margin: '16px 0 8px', fontSize: 11 }}>
                Historique du niveau de satisfaction
              </p>
    ...
    return (
      <div className="goal-row">
        <input
          ...
          autoFocus
        />
        <button className="icon-btn" onClick={save} title="Enregistrer">&#10003;</button>
        <button className="icon-btn" onClick={() => { setLabel(goal.label); setEditing(false) }} title="Annuler">&times;</button>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="goal-row">
        ...
        {canEdit && (
          <>
            <button className="icon-btn" onClick={() => setEditing(true)} title="Modifier">&#9998;</button>
            <button
